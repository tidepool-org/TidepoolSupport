...
struct MyCaregiversView: View {
    ...
    @ViewBuilder
    private func nameStack(for caregiver: Caregiver) -> some View {
    ...
    var confirmationDialogTitle: String {
        guard let selectedCaregiver else {
            return ""
        }
        
        return "Manage account for \(selectedCaregiver.name.isEmpty ? selectedCaregiver.email : selectedCaregiver.name)"
    }
    ...
    var body: some View {
        ZStack(alignment: .bottom) {
                ...
                Section(header: header) {
                    ForEach(caregiverManager.caregivers) { caregiver in
                        ...
                        .alert(
                            LocalizedString(
                                "Remove Caregiver?",
                                comment: "Alert title for remove caregiver confirmation."
                            ),
                            isPresented: Binding(
                                get: {
                                    presentedAlert == .showingRemoveConfirmation
                                ...
                                Text(
                                    String(
                                        format: LocalizedString(
                                            "%1$@ will lose all access to your data. Are you sure you want to remove this caregiver?",
                                            comment: "Format string for message on remove caregiver alert confirmation"
                                        ),
                                        caregiver.name
                                    )
                                )
                        ...
                        .alert(
                            LocalizedString(
                                "Resend Invitation",
                                comment: "Alert title for resend invitation confirmation."
                            ),
                            isPresented: Binding(
                                get: {
                                    presentedAlert == .showingResendConfirmation
                            ...
                            actions: { caregiver in
                                Button(role: .destructive) {
                                    ...
                                    Text(
                                        LocalizedString(
                                            "Resend Invite",
                                            comment: "Button title on alert for resend invitation confirmation"
                                        )
                                    )
                                }
                            },
                            message: { caregiver in
                                Text(
                                    String(
                                        format: LocalizedString(
                                            "Are you sure you want to resend a share invitation to %1$@?",
                                            comment: "Format string for message on resend invitation alert confirmation"
                                        ),
                                        caregiver.name.isEmpty ? caregiver.email : caregiver.name
                                    )
                                )
                            }
                        )
                        .alert(
                            caregiverManager.errorType?.title ?? "Error",
                            isPresented: Binding(
                                get: {
                                    presentedAlert == .showingTroubleDialog
                                }, set: { _ in
                            ...
                            message: {
                                Text(
                                    String(
                                        format: LocalizedString(
                                            "%1$@ Please try again.",
                                            comment: "Format string for message error alert"
                                        ),
                                        caregiverManager.errorType?.description ?? "We weren’t able to complete this action."
                                    )
                                )
                            }
                        )
            ...
            .animation(.default, value: presentedBottomTray)
        }
        .task {
            await caregiverManager.fetchAllCaregivers()
        }
